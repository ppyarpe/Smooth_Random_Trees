...
for total_budget in total_budgets_list:
    forest_testing = DP_Random_Forest(train, test, [2,4,6,7,8,9], ntree, total_budget)
    print(total_budget)
    perf = perf.append({'Budget': total_budget,'Accuracy':forest_testing._accuracy}, ignore_index=True)
    output_path='testing_12v/'+str(ntree)+'_trees/NP/y_pred_full_budget_'+str(total_budget)+'.csv'
    y_pred=pd.DataFrame(forest_testing._predicted_labels)  
    y_pred.to_csv(output_path)


perfy=pd.DataFrame(perf)  
perfy.to_csv('testing_12v/'+str(ntree)+'_trees/NP/performance_metrics_'+str(total_budget)+'.csv')
