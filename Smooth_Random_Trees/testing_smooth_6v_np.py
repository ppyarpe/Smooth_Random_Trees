...
for total_budget in total_budgets_list:
    forest_testing = DP_Random_Forest(train, test, [1,3,4,5], ntree, total_budget)
    print(total_budget)
    perf = perf.append({'Budget': total_budget,'Accuracy':forest_testing._accuracy}, ignore_index=True)
    output_path='testing_6v/'+str(ntree)+'_trees/NP/y_pred_full_budget_'+str(total_budget)+'.csv'
...
perfy.to_csv('testing_6v/'+str(ntree)+'_trees/NP/performance_metrics_'+str(total_budget)+'.csv')
