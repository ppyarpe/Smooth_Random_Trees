...
perf=pd.DataFrame(columns=['Variables','Accuracy','ROC'])
...
for var in variables:
    ...
    output_path='NP_RandomForest/'+str(n_trees)+'_trees/y_pred_'+str(var)+'v.csv'
    pd.DataFrame(y_pred).to_csv(output_path, index=None)
        
    acc=sklearn.metrics.accuracy_score(y_test, y_pred)
    roc=sklearn.metrics.roc_auc_score(y_test,y_pred)
    perf = perf.append({'Variables': var,'Accuracy':acc,'ROC':roc}, ignore_index=True)
    
perf.to_csv('NP_RandomForest/'+str(n_trees)+'_trees/performance_metrics.csv')
