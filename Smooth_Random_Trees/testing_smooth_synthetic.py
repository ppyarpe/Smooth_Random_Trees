...
# threshold<-[5000,10000,15000,20000,25000,30000,35000,40000,45000,50000]

df_1=pd.DataFrame({'gender':'female','salary':np.random.normal(20000,10000,500)})
df_2=pd.DataFrame({'gender':'male','salary':np.random.normal(38000,10000,500)})
dataset_2_i=pd.concat([df_1,df_2])
default=pd.DataFrame((dataset_2_i['salary']<=35000).astype(int))
default.rename(columns={'salary':'default'},inplace=True)
dataset_2_i['thres']=35000
dataset_2=pd.concat([default,dataset_2_i],axis=1)



p=0.5
data_test=dataset_2[(dataset_2['gender']=='female')]
...
for index, row in data_test.iterrows():
    if row['salary']<=35000:
       if random.uniform(0,1)<=p:
           data_test_n.iloc[index,0]=0
       else:
           data_test_n.iloc[index,0]=1
           
    else:
        data_test_n.iloc[index,0]=0
        
bb=data_test=dataset_2[(dataset_2['gender']=='male')]

dataset_2_n=pd.concat([data_test_n,bb])

dataset_2_n.drop(columns=['thres'],inplace=True)

train,test= train_test_split(dataset_2_n, test_size=0.10, random_state=314)

test.to_csv('/Users/anaritapena/Smooth_Random_Trees/Smooth_Random_Trees/Synthetic Datasets/test.csv',index=False)
train.to_csv('/Users/anaritapena/Smooth_Random_Trees/Smooth_Random_Trees/Synthetic Datasets/train.csv',index=False)
...
#     forest_testing = DP_Random_Forest(train, test, [1,], ntree, total_budget)
#     print(total_budget)
...
for total_budget in total_budgets_list:
     ...
     for n_iter in range (1,n_iter_tot+1):
         ...
         output_path='/Users/anaritapena/Smooth_Random_Trees/Smooth_Random_Trees/Synthetic Datasets/y_pred_budget_'+str(total_budget)+'_'+str(n_iter_tot)+'_iter_10_trees.csv'
...
output_path_1='/Users/anaritapena/Smooth_Random_Trees/Smooth_Random_Trees/Synthetic Datasets/performance_metrics_mean_over_'+str(n_iter_tot)+'_iter_10_trees.csv'
...
for n_iter in range (1,n_iter_tot+1):
     forest_testing_np = DP_Random_Forest(train, test, [1,], ntree, 1000000000)
...
y_pred_np_total.to_csv('/Users/anaritapena/Smooth_Random_Trees/Smooth_Random_Trees/Synthetic Datasets/y_pred_total_NP_10_iter_10_trees.csv',index=False)
perf_np_over=pd.DataFrame({'Accuracy':[statistics.mean(perf_np['Accuracy'])],'ROC':[statistics.mean(perf_np['ROC'])]})    
 #perf_np_over={'Accuracy':statistics.mean(perf_np['Accuracy']),'ROC':statistics.mean(perf_np['ROC']) } 
output_path_2='/Users/anaritapena/Smooth_Random_Trees/Smooth_Random_Trees/Synthetic Datasets/performance_metrics_NP_'+str(n_iter_tot)+'_iter_10_trees.csv'
...
plt.axhline(y = perf_np_over.iat[0,0], color = 'grey', linestyle = 'dashed')
plt.axis([0,10,0.5,0.9])
plt.xlabel("Privacy Loss")
plt.legend()
plt.show()
plt.savefig('/Users/anaritapena/Smooth_Random_Trees/Smooth_Random_Trees/Synthetic Datasets/acc_plot.png')
...
plt.axhline(y = perf_np_over.iat[0,1], color = 'grey', linestyle = 'dashed')
plt.axis([0,10,0.5,0.8])
plt.xlabel("Privacy Loss")
plt.legend()
plt.show()
plt.savefig('/Users/anaritapena/Smooth_Random_Trees/Smooth_Random_Trees/Synthetic Datasets/roc_plot.png')
