...
for total_budget in total_budgets_list:
    ...
    output_path='testing_13v/'+str(ntree)+'_trees/NP/y_pred_full_budget_'+str(total_budget)+'.csv'
    y_pred=pd.DataFrame(forest_testing._predicted_labels)  
    y_pred.to_csv(output_path)

perfy=pd.DataFrame(perf)  
perfy.to_csv('testing_13v/'+str(ntree)+'_trees/NP/performance_metrics_'+str(total_budget)+'.csv')
